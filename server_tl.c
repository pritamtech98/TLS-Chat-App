...
void mycall(int w, int v, int c, const void *buff, size_t len, SSL *ssl, void *arg){
    printf("inside server callback\n");
    printf("val of write: %d, val of version : %d, val of content : %d\n", w, v, c);
    char *bu = (char *)buff;
    printf("%s\n", bu);
    printf("\n\n");
}
...
{   SSL_METHOD *method;
    ...
    SSL_CTX_set_msg_callback(ctx, mycall);
    return ctx;
}
